...
const About: React.FC<AboutProps> = ({ imageSrc, title, description }) => {
  ...
  const blobAnimation = useSpring({
    ...
    loop: { reverse: true },
    onRest: () => setAnimationComplete(!animationComplete)
  });
    ...
    style={{
      ...
      <div ref={ref} className="flex items-center bg-[#81d5ff] rounded-lg p-6">
        <div className="relative w-1/3 flex justify-center">
          ...
          <svg
            className="absolute pb-72 -bottom-96"
            id="visual"
            viewBox="0 0 900 600"
            width="900"
            height="600"
          ...
          >
            <defs>
              <radialGradient id="gradient" cx="50%" cy="50%" r="50%">
                <stop offset="30%" style={{ stopColor: "#81d5ff", stopOpacity: 1 }} />
                <stop offset="100%" style={{ stopColor: "#004b90", stopOpacity: 1 }} />
              </radialGradient>
            </defs>
            <g transform="translate(460.5532557363372 261.4922545353779)">
              <animated.path
                id="blob1"
                d={blobAnimation.x}
                fill="url(#gradient)"
